import { sequelize, SQLQueryTypes } from '../../config/database';
import * as dotenv from 'dotenv';
dotenv.config();

const queryDB = async <T = any>(sql: string, params: any[] = []): Promise<T[]> => {
    try {
...
const loadTextGuidsFromVerseIds = async (verse_ids: number[]) => {
    ...
    return text_guids.map((x: any) => x.text_guid);

}

const loadPageSlugFromTextGuid = async (text_guid: string) => {
    ...
    const {link, pageslug, content} = items[0] as any;
