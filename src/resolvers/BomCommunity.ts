...
const loadUserFromToken = async (token: string) => {
  try {
    ...
    console.error('Database error during loadUserFromToken:', error);
    return null;
  }
}
  ...
  Query: {
    ...
    leaderboard: async (item: any, args: any, context: any, info: any) => {
      ...
      try {
        ...
        console.error('Database error during leaderboard:', error);
        return {
          recentFinishers: [],
          currentProgress: [],
        }
      ...
      try {
        ...
        return {
          ...
          userHistories: userList.map((username: string) => ({
            user: username,
            progress: standardizedValues.map((item: any) => {
              const userProgress = item.progress?.[username];
              if (userProgress === undefined || userProgress === null) {
                return 0; // Default to 0 if no progress data
              }
              return Math.round((userProgress * 1000) / textMax) / 10;
            })
          }))
        };
      } catch (error) {
    ...
    homegroups: async (item: any, args: any, context: any, info: any) => {
      ...
      try {
        ...
        console.error('Database error during homegroups:', error);
        return [];
      }
    },
      ...
      try {
        ...
        if (group.custom_type === 'private') {
          if (args.token) {
            ...
            const my_sb_user_id = md5(bom_username);
            let mySBGroups = await sendbird.getMyGroups({ user_id: my_sb_user_id });
            let foundGroup = mySBGroups.filter(g => g.channel_url === args.channel);
            if (!foundGroup || !foundGroup.length) return [];
          } else {
            return [];
          }
        }
        ...
        console.error('Database error during homethread:', error);
        return [];
      }
    },
    homefeed: async (item: any, args: any, context: any, info: any) => {
      ...
      try {
        ...
        console.error('Database error during homefeed:', error);
        return {
          groups: [],
          feed: []
        };
    ...
    requestedUsers: async (item: any, args: any, context: any, info: any) => {
      ...
      try {
        ...
        const data = await sendbird.listUsers(userIds);
        return data.map(sbUser => loadHomeUser(sbUser));
      } catch (error) {
        console.error('Database error during requestedUsers:', error);
        return [];
      }
    },
    ...
    readingplan: async (item: any, args: any, context: any, info: any) => {
      ...
      try {
        ...
        console.error('Database error during readingplan:', error);
        return [];
      }
    },
    readingplansegment: async (item: any, args: any, context: any, info: any) => {
      ...
      try {
        ...
        console.error('Database error during readingplansegment:', error);
        return [];
      }
    }
  },
  Mutation: {
    addBot: async (item: any, args: any, context: any, info: any) => {
      try {
        ...
        console.error('Error during addBot:', error);
        return false;
      }
    },
    removeBot: async (item: any, args: any, context: any, info: any) => {
      try {
        ...
        console.error('Error during removeBot:', error);
        return false;
      }
    },
    joinGroup: async (item: any, args: any, context: any, info: any) => {
      ...
      try {
        ...
        console.error('Database error during joinGroup:', error);
        return false;
      }
    },
    joinOpenGroup: async (item: any, args: any, context: any, info: any) => {
      ...
      try {
        ...
        console.error('Database error during joinOpenGroup:', error);
        return { isSuccess: false, msg: 'Database error' };
      }
    },
    requestToJoinGroup: async (item: any, args: any, context: any, info: any) => {
      ...
      try {
        ...
        console.error('Database error during requestToJoinGroup:', error);
        return { isSuccess: false, msg: 'Database error' };
      }
    },
    withdrawRequest: async (item: any, args: any, context: any, info: any) => {
      ...
      try {
        ...
        console.error('Database error during withdrawRequest:', error);
        return { isSuccess: false, msg: 'Database error' };
      }
    },
    ...
    processRequest: async (item: any, args: any, context: any, info: any) => {
      ...
      try {
        ...
        console.error('Database error during processRequest:', error);
        return false;
      }
    }
  },
...
async function getFeaturedGroups(lang, limit = 20) {
  ...
  try {
    ...
    recentUsers = [...virtualUsers, ...recentUsers]; // Fix: spread recentUsers instead of nesting
    ...
    console.error('Database error during getFeaturedGroups:', error);
    return [];
  }
}
