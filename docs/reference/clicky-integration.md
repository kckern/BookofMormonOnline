...
| `frontend/webapp/public/index.html` (L81–99) | Vendor-neutral `<title>` observer that emits `bom:analytics-pageview`, marking the first meaningful React title as `initial`. It contains no Clicky global or loader code. |
...
1. `index.html` initializes `lastPath`; its vendor-neutral observer emits `bom:analytics-pageview` when React supplies the first meaningful title and on later pathname/title changes. The first event carries `initial: true`.
2. The analytics adapter initializes during the app bootstrap and configures Clicky with only `history_disable`. The initial generic event gives the adapter the resolved title and causes `providers/clicky.js` to inject `<script async data-id="66488278" src="/<CLICKY_JS_PATH>">`. Keeping the automatic initial pageview enabled preserves `document.referrer` without producing a duplicate observer hit. A two-second loader fallback covers a page that never produces a meaningful title.
