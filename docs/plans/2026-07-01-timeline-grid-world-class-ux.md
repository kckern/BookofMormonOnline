...
# ROUND 2 (2026-07-01, KC directive): green-field JSON data + polish audit

**Directive:** remove the GraphQL dependency for this view — bake ALL timeline
data into hardcoded JSON (API-ify later). Green field data-wise: get it right,
all of it. Plus specific defects: over-squared corners, hover brightness
breaking gradients, battles conflated with person labels in the early-Nephite
run, the Shilom overlap, square outlines on circular icons, missing chevrons +
missing gradients on the Join/defector bars.

## Data architecture (supersedes GQL for this view)

New file `frontend/webapp/src/views/Timeline/timelineData.json`:
```json
{ "events": [ { "slug": "...", "heading": "...", "date": "...", "html": "...",
    "textSlug": "...", "kind": "event|place|label",
    "grid": { "row": 0, "col": 0, "rowSpan": 1, "colSpan": 1, "bg": "#hex",
              "bgTo": "#hex?", "anchor": "?", "tier": 0, "dir": "l|r?",
              "icon": "battle?" } } ] }
```
- Generated by `scripts/timeline-grid/gen_timeline_data.py`: GQL dump (:5006,
  one last time) + battle tiles from gridTiles.json (→ icon-events; bound 16
  carry content) + an authored overlay file
  `scripts/timeline-grid/data-overrides.json` (tiers, anchors, dirs, bgTo,
  placement moves, new label rows, kind conversions). Regenerable; the overlay
  is the editorial source of truth.
- `kind:"label"` = pure text (person rosters) — no popover, never clickable.
- `bgTo` = along-bar gradient end (defection/join bars per the source artwork).
- Timeline.js: `timeline` becomes the imported constant (no fetch/loading
  state/cache bypass); `battleSlugs.json` retired (baked); `k:'battle'` tiles
  DELETED from gridTiles.json (icon-events replace them — this closes plan
  Task 7's retirement item without the SQL gate); backend fields stay for
  future API-ification.

## R2 tasks
- **R2a — de-GQL + generator:** timelineData.json generated + wired; battle
  tiles retired; no fetch. App renders identically-or-better with zero network.
- **R2b — data authoring (the "get it right" pass):** band names tier 1;
  early-Nephite person roster authored as `kind:"label"` rows (Nephi, Sam,
  Zoram, Jacob, Enos, Jarom, Omni, Amaron, Abinadom, Amaleki) DISTINCT from the
  `lamanites-vs-*` battles, which convert to icon-events at their battle-tile
  cells; Shilom placement fixed (no overlap); dir + bgTo on
  join/defection/expedition bars; remaining ~49 unplaced content rows placed by
  date+lineage (best-effort, each flagged in the overlay for review).
- **R2c — hover redesign:** replace brightness(1.15) on hovered lineage with
  DIMMING of non-hovered lineages (`opacity/saturate` on `[data-lin]` NOT
  matching) — uniform across flat fills AND gradient tiles.
- **R2d — corner audit:** per-tile `rd` override support (JSON wins over the
  algorithm); audited pass re-rounding the over-squared sites (rule v2 kept as
  default; overrides where KC's "should be rounded" applies).
- **R2e — icon/label polish:** circular focus/selected ring on battle
  medallions (no square outline around round icons); drop title from canvas
  place buttons; handoff doc honesty fix (LOD/type-ramp now truly live via
  baked tiers).
- **R2f — adversarial visual re-check** against prod + source artwork.
