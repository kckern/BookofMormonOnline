...
const CSVURL = "https://docs.google.com/spreadsheets/d/1s8VT3fa1AyKi63TmJkCx2xqvZpRBdqMgr5Pr2Mym_lg/export?format=csv";
...
.then((parsedData)=>{ 
    ...
    let sql = parsedData.map(item=>{
        ...
        let translation = `DELETE FROM bom_translation WHERE guid='${item[0]}' AND lang = 'ko';
            INSERT INTO bom_translation 
            ( \`guid\`, \`lang\`, \`refkey\`, \`value\`, \`contributor\`) 
            VALUES ('${item[0]}','ko','label_text','${item[4]}','kcsql');
        `;
        return `${label}\n${translation}`;

    })
    fs.writeFile('./update.sql', sql.join("\n"), err => {
        if (err) {
          console.error(err)
          return
        }
