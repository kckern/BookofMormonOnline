...
export function StudyHistory({
        ...
        if (result?.studylog) {
          ...
          BoMOnlineAPI(
            ...
            if (result?.userdailyscores) {
              ...
              const mostRecent = seriesData?.[0]?.data
                ?.map((i) => i[1])
                .reverse()[0];
  ...
  return (
    <Card className="historySide" onClick={() => setHistoryView(true)}>
      ...
      <CardBody>
              ...
              {(years || [new Date().getFullYear().toString()]).map((y, i) => (
                <NavItem key={i}>
                  <NavLink
                    ...
                    role="tab"
                    className={
                      horizontalTabs === y ? "yeartab active" : "yeartab"
                    }
        ...
        >
          {(years || [new Date().getFullYear().toString()]).map((y, k) => {
            ...
            for (let i in studyLog) {
              let item = studyLog[i];
              let year = new Date(item.timestamp * 1000)
                .getFullYear()
                .toString();
              if (year !== y) continue;
            ...
            return (
              <TabPane key={k} tabId={y} role="tabpanel">
                {[
      ...
      {progressList && maxValue ? (
        <>
          <CardHeader>
            <h5 className="title">
              <img src={growth} /> {label("study_progress")}
            </h5>
          </CardHeader>
          <CardBody>
            <HighchartsReact
                  ...
                  labels: {
                    formatter: function() {
                      return Highcharts.dateFormat(
                        progressList?.progress?.length > 100
                          ? "%b %Y"
                          : "%d %b %Y",
                        this.value,
                      );
...
function prepareSeriesData(progressList) {
  ...
  let chartdata = progressList.progress.map((val, i) => [
    new Date(progressList.dates[i]) * 1,
    val,
  ]);
  let series_index = 0;
  let lastval = 0;
  ...
  return data.map((series) => ({
    ...
    tooltip: {
      ...
      pointFormatter: function() {
        return `<div>${new Date(this.x).toISOString().slice(0, 10)}: <b>${
          this.y
        }%</b><div>`;
      },
    },
