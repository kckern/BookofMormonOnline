...
export default function User({ appController }) {
  useEffect(
    () => (document.title = label("preferences") + " | " + label("home_title")),
    [],
  );
  ...
  let pubs = publications?.map((publication, i) => (
    <Label
      key={i}
      rating={publication.source_rating}
      onClick={(e) =>
        toggleCommentarySource(e, parseInt(publication.source_id))
      }
    >
      <Card className={"publicationCard"}>
        <CardHeader>
          <input
            ...
            readOnly
            checked={
              !appController.states.preferences.commentary.filter.sources.includes(
                parseInt(publication.source_id),
              )
            }
          />
          <div>
            ...
            <div className="source_publisher">
              © {publication.source_year || 2000},{" "}
              {publication.source_publisher}
            </div>
          </div>
        </CardHeader>
        ...
        <CardBody className={"publicationCardBody"}>
          <img
            src={`${assetUrl}/source/cover/${publication.source_id.padStart(
              3,
              "0",
            )}`}
          />
  ...
  if (appController.preLoad.fax === undefined) return null;

  let faxs = Object.values(appController.preLoad.fax).map((fax, i) => (
    <Label key={i} onClick={(e) => toggleFaxVersion(e, fax.slug)}>
      <Card className={"faxCard"}>
  ...
  return (
    <>
      <Card className={"userPrefs"}>
        ...
        <CardBody>
          <h5 className="title">
            <Label className="audio_narration">
              <img src={audio} />
          ...
          <hr />
          <h5 className="title">
            <Label className="commentary_select">
              <img src={commentary} />
          ...
          <div className={"faxCards"}>
            {appController.states.preferences.facsimiles.on ? (
              faxs ? (
                faxs
              ) : (
                <div>{label("loading_facsimiles")}</div>
              )
            ) : null}
          </div>
        </CardBody>
...
function Publications({ appController, pubs }) {
  ...
  let controversial =
    rpubs && rpubs.length > 0 ? (
      <>
        <Alert color="warning">
          {label("controversial_commentaries_warning")}
        </Alert>
        <div className={"publicationCards"}>{rpubs}</div>
      </>
    ) : null;
