...
export default function MobileUser({ appController }) {
  ...
  const [activeTab, setActiveTab] = useState((appController.states.user.social) ? "2" : "1");
  ...
  return <div className="content mobileuser">
    <div>
      ...
      <TabContent activeTab={activeTab}>
        <TabPane tabId="1">
          <Row>
            <ProfileItems appController={appController} studySummary={studySummary} setHistoryView={() => { }} />
...
function MobileProgressBox({ appController }) {
  ...
  if(!Array.isArray(ProgressBoxData.progressData)) ProgressBoxData.progressData = Object.values(ProgressBoxData.progressData);
  return <div>
  <h4>{label("study_progress_for_x", [name ? name : label("guest")])}</h4>
    <ProgressBar complete={ProgressBoxData.userProgress.completed} started={ProgressBoxData.userProgress.started} />
    <div className='completed_started'>
      {numericLoad(complete)}% {label("completed")} • {numericLoad(started)}% {label("started")}
    </div>
    <hr/>
    ...
    {ProgressBoxData.progressData.map(item => {
      ...
      return <div className='divisionProgressItem' onClick={()=>appController.functions.setPopUp({
          type: "user/progress",
          ids: [item.slug],
          vhtop: 10
        })}>
        ...
        {percentage>=0 ?
         ...
         : <div className='loadingCircle'></div> }</div>
