...
export function loadMenu() {
  var list = [
    ...
    },
    {
      ...
      title: (
        <span>
          <img src={contents} /> {label("menu_contents")}
        </span>
      ),
    },
    {
      ...
      title: (
        <span>
          <img src={study} /> {label("menu_study")}
        </span>
      ),
    },
    {
      ...
      title: (
        <span>
          <img src={theater} /> {label("menu_theater")}
        </span>
      ),
      dev: true,
    },
    {
      ...
      title: (
        <span>
          <img src={timeline} /> {label("menu_timeline")}
        </span>
      ),
    },
    {
      ...
      title: (
        <span>
          <img src={people} /> {label("menu_people")}
        </span>
      ),
    },
    {
      ...
      title: (
        <span>
          <img src={relationships} /> {label("menu_network")}
        </span>
      ),
      dev: true,
    },
    {
      ...
      title: (
        <span>
          <img src={places} /> {label("menu_places")}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      title: (
        <span>
          <img src={historyicon} /> {label("menu_history")}
        </span>
      ),
    },
    {
      ...
      title: (
        <span>
          <img src={analysis} /> {label("menu_analysis")}
        </span>
      ),
      dev: true,
    },
    {
      ...
      title: (
        <span>
          <img src={about} /> {label("menu_about")}
        </span>
      ),
    },
  ];
...
function Sidebar(props) {
  ...
  const determinePath = () => {
    ...
    if (["%ED%8A%B9%EB%B3%84%EB%B0%98", "studyedition"].includes(slugRoot))
      return "/특별반";
      ...
      <div className="sidebar-wrapper">
        ...
        <Nav className="sidebar-menu">
          ...
          {menu.map((r, index) => {
            ...
            return (
              <li
                className={r.slug + "_link  menuitem " + activeClass}
                key={r.slug}
              >
                <NavLink
                  to={"/" + r.slug}
...
function UserInfo({ appController, setActivePath, activePath }) {
  ...
  useEffect(() => {
    ...
    appController.states?.user.progress?.completed,
    appController.states?.user.progress?.started,
  ]);

  let startedString =
    completed > started
      ? ""
      : " • " + (started?.toFixed(1) || 0) + "% " + label("started");

  useEffect(() => {
    if (
      appController.states?.user?.social &&
  ...
  if (
    name === label("loading_user") &&
    appController.sendbird?.sb.currentUser?.user_id
  ) {
      ...
      <li className={activeClass}>
        ...
        <div className="settings">
          ...
          <img
            onClick={toggleSound}
            data-tip={
              appController.states?.preferences.audio
                ? label("audio_on")
                : label("audio_off")
            }
            src={appController.states?.preferences.audio ? soundOn : soundOff}
          />
          <Link to={"/user/preferences"}>
            <img data-tip={label("user_prefs")} src={settings} />
          </Link>
          <Link to={"/user/history"}>
            <img data-tip={label("user_history")} src={lastseen} />
          </Link>
        </div>
      </li>
    </Nav>
