...
export function getStartEnd(level, { key }) {
...
const loadLevelAndKeyFromSlug = (slug) => {
  //check level 1
  const groups = Object.keys({ ...bible, ...bom });
  if (groups.includes(slug)) return { level: "groups", key: slug };
  //check level 2
  const books = Object.values({ ...bible, ...bom }).flat();
  const book = books.find(([name]) => slug === slugify(name));
  if (book) return { level: "books", key: book[0] };
...
function ScriptureGrid() {
    ...
    const [_,parts] = value?.split("/") || [];
    const [front,back] = parts?.split && parts?.split("~") || [];

    const frontData = loadLevelAndKeyFromSlug(front);
    const backData = loadLevelAndKeyFromSlug(back);
