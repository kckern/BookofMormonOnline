 ...
 * Works regardless of whether image is from Sendbird CDN (dead), our CDN, or missing
 */
export default function UserAvatar({ userId, profileUrl, size = 40, className = '', style = {} }) {
  const [failed, setFailed] = useState(false);
  
  // Use fallback if: no URL, load failed, or is a Sendbird URL (CDN is dead)
  const useFallback = !profileUrl || failed || 
    (profileUrl && (profileUrl.includes('sendbird.com') || profileUrl.includes('sendbird.io')));
  
  const finalSrc = useFallback ? generateAvatarUrl(userId) : profileUrl;
