var clicky_obj = clicky_obj || (function() {
    ...
    //return empty object if localhost
    ...
    function _ins() {
        ...
        this.beacon = function(type, q, called_by_pageview) {
            ...
            for (var site_id_index = 0; site_id_index < site_ids.length; site_id_index++) {
                ...
                _self.inject(_self.domain + 
                    ...
                    _self.he_model : '') + 
                    '&ck=' + 
                    (navigator.cookieEnabled && !clicky_custom.cookies_disable ? 1 : 0) + 
                    (hm ? '&hm=' + 
                    hm : '') + 
        ...
        this.set_cookie = function(name, value, expires, force) {
            if ((clicky_custom.cookies_disable && !force) || (clicky_custom.sticky_data_disable && name.match(/^_(custom|utm|referrer)/))) return false;
...
_cgen.init(66488278);
    ...
    console.log(`Stats Running In Dev Mode`);
}