...
const studyBuddy = async (channelUrl,messageId, messageContent) => {
    ...
    if(screen) return error("studyBuddy pre-screened as bad faith", {screen});

    //start typing indicator
    sendbird.startStopTypingIndicator(channelUrl, [studyBuddyId], true);
    log("studyBuddy started typing");

    const result = await studyBuddyTextBlock({channelUrl, messageId, lang, studyBuddyId});
    if (!result || typeof result === 'object' && 'mesg' in result) {
        // Handle error case or void return
        return result;
    }
    const { response, metadata, page_slug} = result;
    if(!response) {
...
const prepareMessages = ({
    ...
    let tokenCount = (openaiTokenCounter as any).chat?.(messagestocount, model) || 0;
    ...
    if(tokenCount>tokenLimit) {
        ...
        if(commentary.length) commentary = commentary.sort((a,b) => b.text.length - a.text.length).slice(1);
            ...
            while(tokenCount > tokenLimit)
            {
                ...
                tokenCount = (openaiTokenCounter as any).chat?.(messagestocount, model) || 0;
                i++;
...
const studyBuddyTextBlock = async ({ channelUrl, messageId, lang, studyBuddyId}) => {
    ...
    const commentary = await loadCommentary(verse_ids);
...
export { studyBuddy, studyBuddyTextBlock, loadSectionNarration, loadSectionContext, loadTextBlockNarration, loadCrossReferences };